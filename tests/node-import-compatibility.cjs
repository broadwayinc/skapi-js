...
fs.writeFileSync(path.join(cjsProject, 'index.js'), [
    ...
    "}",
    "",
    "// The Node storage shim is what Cognito reaches on every signup, login and",
    "// session restore. Importing the package is not enough of a test: the shim",
    "// used to reach for fs through a bare require(), which esbuild turns into a",
    "// throwing stub in the ESM bundle, so the export looked fine and every",
    "// authenticated call died. Exercise a real round trip.",
    "const probe = 'skapi_import_probe';",
    "window.localStorage.setItem(probe, 'value');",
    "if (window.localStorage.getItem(probe) !== 'value') {",
    "  throw new Error('localStorage shim did not round-trip');",
    "}",
    "window.localStorage.removeItem(probe);",
    "if (window.localStorage.getItem(probe) !== null) {",
    "  throw new Error('localStorage shim did not remove the key');",
    "}",
    "if (typeof window.sessionStorage.setItem !== 'function') {",
    "  throw new Error('sessionStorage shim is missing');",
    "}",
    "console.log('CJS_OK');"
].join('\n'));
